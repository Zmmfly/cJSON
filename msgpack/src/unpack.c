...
bool msgpack_unpacker_init(msgpack_unpacker* mpac, size_t initial_buffer_size)
{
    ...
    buffer = (char*)cJSON_malloc(initial_buffer_size);
    if(buffer == NULL) {
        return false;
    }

    ctx = cJSON_malloc(sizeof(template_context));
    if(ctx == NULL) {
        cJSON_free(buffer);
        return false;
    }
...
{
    msgpack_unpacker* mpac = (msgpack_unpacker*)cJSON_malloc(sizeof(msgpack_unpacker));
    if(mpac == NULL) {
        return NULL;
    }
...
bool msgpack_unpacker_expand_buffer(msgpack_unpacker* mpac, size_t size)
{
    ...
    if(mpac->off == COUNTER_SIZE) {
        ...
        tmp = (char*)cJSON_realloc(mpac->buffer, next_size);
        if(tmp == NULL) {
            return false;
        }
        ...
        tmp = (char*)cJSON_malloc(next_size);
        if(tmp == NULL) {
            return false;
        }
